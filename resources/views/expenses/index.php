                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <option value="">Select One</option>                              
                                         <?php foreach ($month as $p):?>
                                        <option value="<?php echo $p->id;?>"><?php echo $p->month;?></option>
                                      <?php endforeach;?>
                                      </select>
                                </div>
                            </div> 
                    ...
                    <div id="table" class="box-body">
                        ...
                        <div id="showtable" class="box-body">
                            <table id="level-table" class="table table-striped table-bordered">
                                <tr><th>ID</th><th> Year</th><th>Month</th><th>Type</th><th>Price</th><th>ACTIONS</th></tr>
 ...
 $("#exp").addClass('active');
    ...
    function table() {
        ...
        $.ajax({
            ...
            method: 'get',
            url: baseurl + "/expenses/list?entry=" + entry + "&page=" + page + "&search=" + search,
    ...
    function searchClicked(e) {
        ...
        $.ajax({
            ...
            method: 'get',
            url: baseurl + "/expenses/list?entry=" + entry + "&search=" + search,
            success: function (response) {
                createTable(response);
            },
            fail: function () {
    ...
    function createTable(resp) {
        ...
        for (var i in data) {
            ...
            row.insertCell(1).innerHTML = data[i].years;
            row.insertCell(2).innerHTML = data[i].mnth;
    ...
    function formSubmit(e) {
        ...
        if ($('#id').val() == "") {
            ...
            var url = baseurl + "/expenses/creates";
        } else {
            var url = baseurl + "/expenses/updates/" + id;
        }
        $.ajax({
            method: "POST",
            url: url,
    ...
    function delt(id) {
        ...
        if (conf) {
            $.ajax({
                method: 'get',
                url: baseurl + "/expenses/deletes/" + id,
                success: function (resp) {
                    var a = JSON.parse(resp);
                    toast(a);
                    table();
